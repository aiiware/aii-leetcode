...
func zigzagLevelOrderBFS(root *TreeNode) [][]int {
	if root == nil {
		return [][]int{}
	}
...
func zigzagLevelOrderTwoStacks(root *TreeNode) [][]int {
	...
	result := [][]int{}
	currentLevel := []*TreeNode{root} // Use as stack
	leftToRight := true

	for len(currentLevel) > 0 {
		levelSize := len(currentLevel)
		level := make([]int, 0, levelSize)
		nextLevel := []*TreeNode{} // Use as stack for next level
...
func dfsZigzag(node *TreeNode, level int, result *[][]int) {
	if node == nil {
		return
	}
