# Documentation Index

This directory contains all project documentation organized by category.

## 📚 Documentation Categories

### [Project Documentation](./project/)
Core project documentation and guides.
- `README.md` - Main project README
- `QUICK_START.md` - Quick start guide
- `AGENTS.md` - AI agent instructions

### [Status Reports](./status/)
Project status and progress reports.
- `PROJECT_STATUS.md` - Current project status
- `PROJECT_COMPLETION_REPORT.md` - Completion report
- `REORGANIZATION_STATUS.md` - Reorganization status
- `REORGANIZATION_SUMMARY.md` - Reorganization summary
- `CLEANUP_SUMMARY.md` - Folder structure cleanup summary

### [Indexes](./indexes/)
Problem and solution indexes.
- `INDEX.md` - Main problem index
- `PROBLEM_INDEX.md` - Problem index
- `DP_SOLUTIONS_INDEX.md` - DP solutions index

### [Dynamic Programming](./dp/)
Dynamic programming documentation.
- `DP_ANALYSIS_SUMMARY.md` - DP analysis summary
- `DP_PATTERN_ANALYSIS.md` - DP pattern analysis
- `DP_PRACTICE_EXERCISES.md` - DP practice exercises
- `DP_QUICK_REFERENCE.md` - DP quick reference

### [Implementation](./implementation/)
Implementation details and summaries.
- `IMPLEMENTATION_SUMMARY.md` - Implementation summary
- `FINAL_PROJECT_SUMMARY.md` - Final project summary

### [Backlog](./backlog/)
Project planning and backlog.
- `BACKLOG.md` - Project backlog and TODO items

### [Plans](./plans/)
Project plans and roadmaps.
- Various planning documents

## 📁 Directory Structure

```
docs/
├── README.md              # This file
├── project/              # Core project docs
├── status/               # Status reports
├── indexes/              # Problem indexes
├── dp/                   # DP documentation
├── implementation/       # Implementation docs
├── backlog/             # Backlog and planning
└── plans/               # Project plans
```

## 🔗 Quick Links

- [Main README](../README.md) - Symlink to docs/project/README.md
- [Quick Start](./project/QUICK_START.md)
- [Problem Index](./indexes/INDEX.md)
- [Project Status](./status/PROJECT_STATUS.md)
- [Backlog](./backlog/BACKLOG.md)
- [Cleanup Summary](./status/CLEANUP_SUMMARY.md)

---

*Last updated: January 2026*