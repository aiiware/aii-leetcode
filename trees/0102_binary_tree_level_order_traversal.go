...
func levelOrderBFS(root *utils.TreeNode) [][]int {
	if root == nil {
		return [][]int{}
	}
...
func levelOrderDFS(root *utils.TreeNode) [][]int {
	result := [][]int{}
	dfs(root, 0, &result)
	return result
}
...
func dfs(node *utils.TreeNode, level int, result *[][]int) {
	if node == nil {
		return
	}
...
func levelOrderTwoQueues(root *utils.TreeNode) [][]int {
	if root == nil {
		return [][]int{}
	}

	result := [][]int{}
	currentLevel := []*utils.TreeNode{root}

	for len(currentLevel) > 0 {
		level := make([]int, 0, len(currentLevel))
		nextLevel := []*utils.TreeNode{}
...
// Time complexity: O(n), Space complexity: O(n)
func levelOrderRecursiveWithQueue(root *utils.TreeNode) [][]int {
	if root == nil {
		return [][]int{}
	}
...
func processLevel(nodes []*utils.TreeNode, result *[][]int) {
	if len(nodes) == 0 {
		return
	}
...
// Time complexity: O(n), Space complexity: O(n)
func levelOrderOptimized(root *utils.TreeNode) [][]int {
	if root == nil {
		return [][]int{}
	}
