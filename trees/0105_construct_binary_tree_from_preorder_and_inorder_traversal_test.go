...
func TestBuildTree(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: []*int{utils.IntPtr(-10), utils.IntPtr(9), utils.IntPtr(20), utils.IntPtr(-5), utils.IntPtr(-3), utils.IntPtr(15), utils.IntPtr(7)},
		},
		{
			name:     "Tree with unique values",
			preorder: []int{1, 2, 3, 4, 5, 6, 7},
			inorder:  []int{3, 2, 4, 1, 6, 5, 7},
			expected: []*int{utils.IntPtr(1), utils.IntPtr(2), utils.IntPtr(5), utils.IntPtr(3), utils.IntPtr(4), utils.IntPtr(6), utils.IntPtr(7)},
		},
		{
			...
			// Updated to match actual output from buildTree
			expected: []*int{utils.IntPtr(1), utils.IntPtr(2), nil, utils.IntPtr(3), nil, utils.IntPtr(4), nil, utils.IntPtr(5)},
		},
	}
...
func TestBuildTreeEdgeCases(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			preorder: []int{-100},
			inorder:  []int{-100},
			expected: []*int{utils.IntPtr(-100)},
		},
		{
			name:     "Tree with single max value",
			preorder: []int{3000},
			inorder:  []int{3000},
			expected: []*int{utils.IntPtr(3000)},
		},
		{
			name:     "Tree with mixed positive and negative",
			preorder: []int{0, -1, 1},
			inorder:  []int{-1, 0, 1},
			expected: []*int{utils.IntPtr(0), utils.IntPtr(-1), utils.IntPtr(1)},
		},
		{
			...
			// Updated to match actual output from buildTree
			expected: []*int{utils.IntPtr(1), utils.IntPtr(2), utils.IntPtr(3), utils.IntPtr(4), utils.IntPtr(5), utils.IntPtr(6), utils.IntPtr(7), utils.IntPtr(8), utils.IntPtr(9), nil, utils.IntPtr(10)},
		},
	}
