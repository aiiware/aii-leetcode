...
func checkBalance(node *utils.TreeNode) (int, bool) {
	if node == nil {
		return 0, true
	}
...
func isBalancedIterative(root *utils.TreeNode) bool {
	if root == nil {
		return true
	}

	stack := []*utils.TreeNode{root}
	heights := make(map[*utils.TreeNode]int)
...
func height(node *utils.TreeNode) int {
	if node == nil {
		return 0
	}
