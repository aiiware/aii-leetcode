...
func createIntersectingLists(listA, listB []int, intersectVal int, skipA, skipB int) (*utils.ListNode, *utils.ListNode, *utils.ListNode) {
	if intersectVal == 0 {
		// No intersection
		return utils.NewListFromSlice(listA), utils.NewListFromSlice(listB), nil
	}

	// Create list A up to intersection point
	var headA, tailA *utils.ListNode
	if len(listA) > 0 {
		headA = &utils.ListNode{Val: listA[0]}
		current := headA
		for i := 1; i < skipA; i++ {
			current.Next = &utils.ListNode{Val: listA[i]}
	...
	var headB, tailB *utils.ListNode
	if len(listB) > 0 {
		headB = &utils.ListNode{Val: listB[0]}
		current := headB
		for i := 1; i < skipB; i++ {
			current.Next = &utils.ListNode{Val: listB[i]}
			current = current.Next
		}
		tailB = current
	}
	...
	if intersectVal > 0 {
		intersectNode = &utils.ListNode{Val: intersectVal}
		if tailA != nil {
