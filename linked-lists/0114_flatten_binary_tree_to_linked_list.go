...
// - The "linked list" should use the same utils.TreeNode class where the right child
...
func flattenIterative(root *utils.TreeNode) {
	if root == nil {
		return
	}
...
func flattenMorris(root *utils.TreeNode) {
	current := root
...
func flattenReversePostOrder(root *utils.TreeNode) {
	var prev *utils.TreeNode
