...
func reverseList(head *utils.ListNode, k int) *utils.ListNode {
...
func ReverseKGroupIterative(head *utils.ListNode, k int) *utils.ListNode {
	if head == nil || k == 1 {
		return head
	}
...
func getKthNode(start *utils.ListNode, k int) *utils.ListNode {
	current := start
...
func reverseListIterative(start, end *utils.ListNode) {
	var prev *utils.ListNode
	current := start
	stop := end.Next
