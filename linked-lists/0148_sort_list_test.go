...
func createList(nums []int) *utils.ListNode {
	if len(nums) == 0 {
		return nil
	}
	
	head := &utils.ListNode{Val: nums[0]}
	current := head
	
	for i := 1; i < len(nums); i++ {
		current.Next = &utils.ListNode{Val: nums[i]}
...
func listToSlice(head *utils.ListNode) []int {
	var result []int
	current := head
...
func copyList(head *utils.ListNode) *utils.ListNode {
	if head == nil {
		return nil
	}
	
	newHead := &utils.ListNode{Val: head.Val}
	current := newHead
	original := head.Next
