...
func MaxProfitIII(prices []int) int {
	...
	for i := 1; i < n; i++ {
		...
		leftProfits[i] = utils.Max(leftProfits[i-1], prices[i]-minPrice)
	}
	...
	for i := n - 2; i >= 0; i-- {
		maxPrice = utils.Max(maxPrice, prices[i])
		rightProfits[i] = utils.Max(rightProfits[i+1], maxPrice-prices[i])
	}
