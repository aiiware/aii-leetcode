...
func hasPathSumRecursive(root *utils.TreeNode, targetSum int) bool {
	if root == nil {
		return false
	}
...
// Time complexity: O(n), Space complexity: O(h) where h is the height of the tree
func hasPathSumDFS(root *utils.TreeNode, targetSum int) bool {
	if root == nil {
		return false
	}
...
func hasPathSumBFS(root *utils.TreeNode, targetSum int) bool {
	if root == nil {
		return false
	}

	nodeQueue := []*utils.TreeNode{root}
...
func hasPathSumOptimized(root *utils.TreeNode, targetSum int) bool {
	return hasPathSumHelper(root, 0, targetSum)
}

func hasPathSumHelper(node *utils.TreeNode, currentSum, targetSum int) bool {
	if node == nil {
		return false
	}
