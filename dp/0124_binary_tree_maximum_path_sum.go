...
func MaxPathSum(root *utils.TreeNode) int {
	...
	dfs = func(node *utils.TreeNode) int {
		...
		leftSum := utils.Max(0, dfs(node.Left))  // Only take positive contributions
		rightSum := utils.Max(0, dfs(node.Right)) // Only take positive contributions
		...
		return node.Val + utils.Max(leftSum, rightSum)
	}
