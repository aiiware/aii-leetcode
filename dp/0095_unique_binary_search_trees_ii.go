...
func generateTrees(n int) []*utils.TreeNode {
	if n == 0 {
		return []*utils.TreeNode{}
	}
	memo := make(map[[2]int][]*utils.TreeNode)
	return generateTreesHelper(1, n, memo)
}

func generateTreesHelper(start, end int, memo map[[2]int][]*utils.TreeNode) []*utils.TreeNode {
	// Check memo
	key := [2]int{start, end}
...
func generateTreesDP(n int) []*utils.TreeNode {
	if n == 0 {
		return []*utils.TreeNode{}
	}

	// dp[i] stores all BSTs for i nodes (values 1..i)
	dp := make([][]*utils.TreeNode, n+1)
	dp[0] = []*utils.TreeNode{nil} // Empty tree

	// Build up from 1 to n
	for length := 1; length <= n; length++ {
		var trees []*utils.TreeNode
...
func cloneAndAdd(root *utils.TreeNode, offset int) *utils.TreeNode {
	if root == nil {
		return nil
	}
	return &utils.TreeNode{
...
func generateTreesIterative(n int) []*utils.TreeNode {
	if n == 0 {
		return []*utils.TreeNode{}
	}

	// Start with list containing only nil (empty tree)
	trees := [][]*utils.TreeNode{{nil}}

	// Build trees of increasing size
	for length := 1; length <= n; length++ {
		var currentTrees []*utils.TreeNode
		...
		for rootVal := 1; rootVal <= length; rootVal++ {
			...
			for _, left := range leftTrees {
				for _, right := range rightTrees {
					...
					currentTrees = append(currentTrees, &utils.TreeNode{
						Val:   rootVal,
						Left:  left,
						Right: adjustedRight,
					})
...
func generateTreesBacktracking(n int) []*utils.TreeNode {
	if n == 0 {
		return []*utils.TreeNode{}
	}
...
func buildBSTFromPermutation(values []int) *utils.TreeNode {
	if len(values) == 0 {
		return nil
	}
	
	root := &utils.TreeNode{Val: values[0]}
...
func insertIntoBST(root *utils.TreeNode, val int) {
	if val < root.Val {
		if root.Left == nil {
...
func isUniqueBST(root *utils.TreeNode, existing []*utils.TreeNode) bool {
...
func treesEqual(a, b *utils.TreeNode) bool {
	if a == nil && b == nil {
		return true
	}
	if a == nil || b == nil {
...
func generateTreesCatalan(n int) []*utils.TreeNode {
	if n == 0 {
		return []*utils.TreeNode{}
	}
	...
	var generate func(int, int) []*utils.TreeNode
	generate = func(start, end int) []*utils.TreeNode {
		var result []*utils.TreeNode
		...
		for i := start; i <= end; i++ {
			...
			for _, left := range leftTrees {
				for _, right := range rightTrees {
					result = append(result, &utils.TreeNode{
						Val:   i,
						Left:  left,
						Right: right,
					})
...
func generateTreesOptimized(n int) []*utils.TreeNode {
	if n == 0 {
		return []*utils.TreeNode{}
	}
	...
	if n < len(catalan) {
		...
		result := make([]*utils.TreeNode, 0, catalan[n])
		memo := make(map[[2]int][]*utils.TreeNode)
